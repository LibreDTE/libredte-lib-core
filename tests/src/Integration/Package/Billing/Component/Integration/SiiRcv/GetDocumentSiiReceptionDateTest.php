...
class GetDocumentSiiReceptionDateTest extends TestCase
{
    ...
    public function testGetDocumentSiiReceptionDate(): void
    {
        $response = $this->siiRcvWorker->getDocumentSiiReceptionDate(
            $this->siiRequest,
            '76192083-9',
            33,
            747
        ...
        $this->assertInstanceOf(GetDocumentSiiReceptionDateResponse::class, $response);
        $this->assertMatchesRegularExpression(
            '/^\d{4}-\d{2}-\d{2} \d{2}:\d{2}:\d{2}$/',
            $response->getReceptionDate()
        );
    }
}
