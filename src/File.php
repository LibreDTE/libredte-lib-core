...
class File
{
     ...
     */
    public static function compress(string $file, array $options = [])
    {
        ...
        if (!is_readable($file)) {
            Log::write(
                Estado::COMPRESS_ERROR_READ,
                Estado::get(Estado::COMPRESS_ERROR_READ)
            );
            return false;
        }
        ...
        if ($options['format'] == 'zip') {
            ...
            file_put_contents(
                $dir . DIRECTORY_SEPARATOR . $file_compressed,
                $zip->getStream(md5($filepath))
            );
            $zip->close();
        } else {
        ...
        // enviar archivo
        if ($options['download']) {
            $filename = $dir . DIRECTORY_SEPARATOR . $file_compressed;
