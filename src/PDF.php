...
class PDF extends \TCPDF
{
    ...
    /**
     * Controla el uso del formato de papel continuo en la impresión de documentos electrónicos.
     *
     * Este atributo indica si se debe generar documentos en formato de papel continuo.
     * Un valor de 0 indica que se utilizará el formato estándar, mientras que cualquier otro valor
     * especifica el ancho del papel continuo que se debe utilizar en milímetros.
     * Es útil para la impresión de boletas electrónicas y facturas en impresoras térmicas.
     *
     * @var int
     */
    protected $papelContinuo = 0;

    /**
     * Mensaje personalizado a incluir en el pie de página de los documentos electrónicos.
     *
     * Este texto se añadirá al final de los documentos generados, como facturas o boletas electrónicas,
     * permitiendo incluir información adicional como datos de contacto, promociones o mensajes legales.
     *
     * @var string|null
     */
    ...
    public function __construct($o = 'P', $u = 'mm', $s = 'LETTER', $top = 0)
    {
        ...
        $this->SetAuthor('LibreDTE - https://www.libredte.cl');
        $this->SetCreator('LibreDTE: Biblioteca (Núcleo)');
    }
    ...
    {
        if (is_array($this->footer) && (!empty($this->footer['left']) || !empty($this->footer['right']))) {
            $style = [
                'width' => 0.5,
                'cap' => 'butt',
                'join' => 'miter',
     ...
     */
    private function addHTMLTable($headers, $data, array $options = [])
    {
        $width_table = (isset($options['width_table']) && !empty($options['width_table']))
            ? ('width:' . $options['width_table'] . '%;') : '';
        ...
        foreach ($headers as $col) {
            ...
            $buffer .= '<th style="border-right:1px solid #333;border-bottom:1px solid #333;text-align:'
                . $align . $width . '"><strong>' . strip_tags($col) . '</strong></th>';
            $i++;
        }
        $buffer .= '</tr>';
        if ($thead) {
        ...
        foreach ($data as &$row) {
            ...
            foreach ($row as $col) {
                ...
                $align = isset($a[$i]) ? $a[$i] : 'center';
                $buffer .= '<td style="border-right:1px solid #333;text-align:'
                    . $align . $width .'">' . $col . '</td>';
                $i++;
            }
            $buffer .= '</tr>';
