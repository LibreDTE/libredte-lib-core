...
class Dte
{
     ...
     */
    public function __construct($datos, bool $normalizar = true)
    {
        if (is_array($datos)) {
     ...
     * @return bool =true si se pudo cargar el XML, =false si no se pudo cargar.
     */
    private function loadXML(string $xml)
    {
        if (!empty($xml)) {
            $this->xml = new \libredte\lib\XML();
            if (!$this->xml->loadXML($xml) || !$this->schemaValidate()) {
                \libredte\lib\Log::write(
                    \libredte\lib\Estado::DTE_ERROR_LOADXML,
                    \libredte\lib\Estado::get(\libredte\lib\Estado::DTE_ERROR_LOADXML)
                );
                return false;
            }
     ...
     * @param normalizar Si se pasa un arreglo permitirá indicar si el mismo se debe o no normalizar.
     */
    private function setDatos(array $datos, bool $normalizar = true)
    {
        if (!empty($datos)) {
            ...
            // armar XML
            $this->xml = (new \libredte\lib\XML())->generate([
                'DTE' => [
                    '@attributes' => [
                        'version' => '1.0',
                    ],
     ...
     * @return array Arreglo con datos del DTE.
     */
    public function getDatos($force_reload = false)
    {
        if (!$this->datos || $force_reload) {
            $datos = $this->xml->toArray();
            if (!isset($datos['DTE'][$this->tipo_general])) {
                \libredte\lib\Log::write(
                    \libredte\lib\Estado::DTE_ERROR_GETDATOS,
                    \libredte\lib\Estado::get(\libredte\lib\Estado::DTE_ERROR_GETDATOS)
                );
                return false;
            }
     ...
     * @return array Arreglo con datos de la firma.
     */
     ...
     * @return string|false JSON con los datos del documento.
     */
     ...
     * @return string ID del DTE.
     */
    public function getID($estandar = false): string
    {
        return $estandar ? ('T' . $this->tipo . 'F' . $this->folio) : $this->id;
    }

    /**
     * Método que entrega el tipo general de documento, de acuerdo al
     * atributo $tipos del DTE.
     * @param dte Tipo númerico de DTE, ejemplo: 33 (factura electrónica).
     * @return string|false Tipo general del DTE: Documento, Liquidacion o Exportaciones.
     */
    private function getTipoGeneral($dte)
    {
        ...
        \libredte\lib\Log::write(
            \libredte\lib\Estado::DTE_ERROR_TIPO,
            \libredte\lib\Estado::get(\libredte\lib\Estado::DTE_ERROR_TIPO, $dte)
        );
        return false;
    }
     ...
     * @return int Tipo de dte, ej: 33 (factura electrónica).
     */
     ...
     * @return int Folio del DTE.
     */
    public function getFolio(): int
    {
        return (int)$this->folio;
    }

    /**
     * Método que entrega RUT del emisor del DTE.
     * @return string|false RUT del emisor del DTE (sin puntos con guión).
     */
     ...
     * @return string|false RUT del receptor del DTE (sin puntos con guión).
     */
    ...
    /**
     * Método que entrega la fecha de emisión del DTE.
     * @return string|false Fecha de emisión del DTE en formato AAAA-MM-DD.
     */
     ...
     * @return float Monto total del DTE (flot por los DTE de exportación).
     */
    public function getMontoTotal(): float
    {
        ...
        return (float)$this->datos['Encabezado']['Totales']['MntTotal'];
    }

    /**
     * Método que entrega el tipo de moneda del documento.
     * @return string|false Tipo de moneda del DTE.
     */
     ...
     * @return array|false Arreglo con las referencias o false si no se pueden obtener.
     */
    ...
    public function getTED()
    {
        ...
        return mb_detect_encoding($TED, ['UTF-8', 'ISO-8859-1']) != 'ISO-8859-1'
            ? mb_convert_encoding($TED, 'ISO-8859-1', 'UTF-8')
            : $TED
        ;
    }
     ...
     * @return bool|null =true si el DTE es de certificación, =null si no se pudo determinar.
     */
    public function getCertificacion()
    {
        $datos = $this->getDatos();
        $idk = !empty($datos['TED']['DD']['CAF']['DA']['IDK'])
            ? (int)$datos['TED']['DD']['CAF']['DA']['IDK']
            : null
        ;
        if (!$idk) {
            return null;
     ...
     * @return bool =true si se pudo timbrar o =false en caso de error.
     */
    public function timbrar(Folios $Folios): bool
    {
        ...
        // Esta validación NO verifica si el folio ya fue usado, solo si está
        ...
        if ($folio < $Folios->getDesde() || $folio > $Folios->getHasta()) {
            \libredte\lib\Log::write(
                \libredte\lib\Estado::DTE_ERROR_RANGO_FOLIO,
                \libredte\lib\Estado::get(\libredte\lib\Estado::DTE_ERROR_RANGO_FOLIO, $this->getID())
            );
            return false;
        }
        ...
        if (!$this->xml->xpath('/DTE/'.$this->tipo_general.'/Encabezado/IdDoc/FchEmis')->item(0)) {
            \libredte\lib\Log::write(
                \libredte\lib\Estado::DTE_FALTA_FCHEMIS,
                \libredte\lib\Estado::get(\libredte\lib\Estado::DTE_FALTA_FCHEMIS, $this->getID())
            );
            \libredte\lib\Log::write('Falta FchEmis del DTE '.$this->getID());
            return false;
        }
        if (!$this->xml->xpath('/DTE/'.$this->tipo_general.'/Encabezado/Totales/MntTotal')->item(0)) {
            \libredte\lib\Log::write(
                \libredte\lib\Estado::DTE_FALTA_MNTTOTAL,
                \libredte\lib\Estado::get(\libredte\lib\Estado::DTE_FALTA_MNTTOTAL, $this->getID())
            );
            return false;
        }
        // timbrar
        ...
        if (!openssl_sign($DD, $timbre, $Folios->getPrivateKey(), OPENSSL_ALGO_SHA1)) {
            \libredte\lib\Log::write(
                \libredte\lib\Estado::DTE_ERROR_TIMBRE,
                \libredte\lib\Estado::get(\libredte\lib\Estado::DTE_ERROR_TIMBRE, $this->getID())
            );
            return false;
        }
     ...
     * @return bool =true si el DTE pudo ser fimado o =false si no se pudo firmar.
     */
    public function firmar(\libredte\lib\FirmaElectronica $Firma): bool
    {
        $parent = $this->xml->getElementsByTagName($this->tipo_general)->item(0);
        ...
        if (!$xml) {
            \libredte\lib\Log::write(
                ...
                \libredte\lib\Estado::get(\libredte\lib\Estado::DTE_ERROR_FIRMA, $this->getID())
            );
            return false;
        }
        $this->loadXML($xml);
     ...
     * @return string XML con el DTE (podría ser con o sin timbre y con o sin firma).
     */
     ...
     * @return array Arreglo con el resumen del DTE.
     */
    public function getResumen(): array
    {
        ...
        foreach ($montos as $dest => $orig) {
            if (!empty($this->datos['Encabezado']['Totales'][$orig])) {
                $resumen[$dest] = !$this->esExportacion()
                    ? round($this->datos['Encabezado']['Totales'][$orig])
                    : $this->datos['Encabezado']['Totales'][$orig]
                ;
            }
        }
        ...
        if ($this->esBoleta()) {
            if (!$resumen['TasaImp']) {
                $resumen['TasaImp'] = \libredte\lib\Sii::getIVA();
            }
            $resumen['MntExe'] = (int)$resumen['MntExe'];
            if (!$resumen['MntNeto']) {
                list($resumen['MntNeto'], $resumen['MntIVA']) = $this->calcularNetoIVA(
                    $resumen['MntTotal'] - $resumen['MntExe'],
                    $resumen['TasaImp']
                );
            }
        }
     ...
     * @return array Arreglo con el neto y el IVA.
     */
    private function calcularNetoIVA($total, $tasa = null)
    {
        ...
        // este tipo de casos debido a las aproximaciones se consideran
        // "normales" y parte de lo que puede pasar con los DTE.
    ...
    private function normalizar(array &$datos)
    {
        ...
        $datos = \libredte\lib\Arreglo::mergeRecursiveDistinct([
            'Encabezado' => [
                'IdDoc' => [
                    'TipoDTE' => false,
                    'Folio' => false,
        ...
        if (!in_array($datos['Encabezado']['IdDoc']['TipoDTE'], [39, 41, 110, 111, 112]) && empty($datos['Encabezado']['IdDoc']['TpoTranVenta'])) {
        ...
        if (!empty($datos['Encabezado']['OtraMoneda'])) {
            ...
            foreach ($datos['Encabezado']['OtraMoneda'] as &$OtraMoneda) {
                ...
                foreach (['MntNeto', 'MntExe', 'IVA', 'IVANoRet'] as $monto) {
                    if (empty($OtraMoneda[$monto.'OtrMnda']) && !empty($datos['Encabezado']['Totales'][$monto])) {
                        $OtraMoneda[$monto.'OtrMnda'] = round(
                            $datos['Encabezado']['Totales'][$monto] * $OtraMoneda['TpoCambio'],
                            4
                        );
                    }
                }
    ...
    private function normalizar_52(array &$datos)
    {
        ...
        if ($datos['Encabezado']['IdDoc']['IndTraslado'] == 5) {
            if (!$datos['Encabezado']['Receptor'] || $datos['Encabezado']['Receptor']['RUTRecep'] != $datos['Encabezado']['Emisor']['RUTEmisor']) {
    ...
    public function normalizar_exportacion(array &$datos)
    {
        ...
        if (empty($datos['Encabezado']['Transporte']['Aduana']['CodModVenta']) && (!isset($datos['Encabezado']['IdDoc']['IndServicio']) || !in_array($datos['Encabezado']['IdDoc']['IndServicio'], [3, 4, 5]))) {
        ...
        if (!empty($datos['Encabezado']['IdDoc']['FmaPagExp']) && $datos['Encabezado']['IdDoc']['FmaPagExp'] == 21 && !empty($datos['Encabezado']['OtraMoneda'])) {
            if (!isset($datos['Encabezado']['OtraMoneda'][0])) {
                $datos['Encabezado']['OtraMoneda'] = [$datos['Encabezado']['OtraMoneda']];
            }
            foreach ($datos['Encabezado']['OtraMoneda'] as &$OtraMoneda) {
    ...
    private function normalizar_detalle(array &$datos)
    {
        ...
        $sumarMontoNF = (
            ...
            || $datos['Encabezado']['Totales']['MontoNF'] === false
        );
        foreach ($datos['Detalle'] as &$d) {
            ...
            if (is_array($d['CdgItem'])) {
                ...
                if ($d['Retenedor'] === false && $d['CdgItem']['TpoCodigo'] == 'CPCS') {
                    $d['Retenedor'] = true;
                }
            }
            ...
            if ($d['PrcItem']) {
                ...
                if (empty($d['MontoItem'])) {
                    ...
                    // aplicar descuento
                    // TODO: Revisar como se aplican descuentos y recargos, ¿debería ser un porcentaje del monto original?
                    if ($d['DescuentoPct']) {
            ...
            if ($d['MontoItem']) {
                ...
                // si es boleta
                    ...
                    // si es afecto, solo agregar al monto total
                    else {
                        $datos['Encabezado']['Totales']['MntTotal'] += $d['MontoItem'];
                    }
                }
    ...
    private function normalizar_aplicar_descuentos_recargos(array &$datos)
    {
        if (!empty($datos['DscRcgGlobal'])) {
            ...
            foreach ($datos['DscRcgGlobal'] as &$dr) {
                ...
                $valor = $dr['TpoValor'] == '%'
                    ? $this->round(
                        ($dr['ValorDR'] / 100) * $datos['Encabezado']['Totales'][$monto],
                        $datos['Encabezado']['Totales']['TpoMoneda']
                    )
                    : $dr['ValorDR']
                ;
                ...
                if ($this->getTipo() == 41 && isset($dr['IndExeDR']) && $dr['IndExeDR'] == 1) {
    ...
    private function normalizar_impuesto_retenido(array &$datos)
    {
        ...
        if (isset($datos['Encabezado']['Totales']['ImptoReten']) && is_array($datos['Encabezado']['Totales']['ImptoReten'])) {
    ...
    private function normalizar_agregar_IVA_MntTotal(array &$datos)
    {
        ...
        if (
            $this->esBoleta() &&
            (
                empty($datos['Encabezado']['IdDoc']['IndMntNeto']) ||
                $datos['Encabezado']['IdDoc']['IndMntNeto'] != 2
            )
        ) {
            $MntTotal = (int)$datos['Encabezado']['Totales']['MntTotal'];
            $MntExe = (int)$datos['Encabezado']['Totales']['MntExe'];
            $total = $MntTotal - $MntExe;
            if (
                $total &&
                (
                    empty($datos['Encabezado']['Totales']['MntNeto']) ||
                    empty($datos['Encabezado']['Totales']['IVA'])
                )
            ) {
                list(
        ...
        if (!empty($datos['Encabezado']['Totales']['MntNeto'])) {
            if ($datos['Encabezado']['IdDoc']['MntBruto'] == 1) {
                ...
                if (
                    empty($datos['Encabezado']['Totales']['IVA']) &&
                    !empty($datos['Encabezado']['Totales']['TasaIVA'])
                ) {
                    $datos['Encabezado']['Totales']['IVA'] = round(
                        $datos['Encabezado']['Totales']['MntNeto'] * (
                            $datos['Encabezado']['Totales']['TasaIVA'] / 100
                        )
                    );
                }
            }
            ...
            if (
                ...
                !empty($datos['Encabezado']['Totales']['MntExe'])
            ) {
                $datos['Encabezado']['Totales']['MntTotal'] = $datos['Encabezado']['Totales']['MntExe'];
            }
        }
            ...
            foreach ($datos['Encabezado']['Totales']['ImptoReten'] as &$ImptoReten) {
                ...
                if (ImpuestosAdicionales::getTipo($ImptoReten['TipoImp']) == 'R') {
                    ...
                    if ($ImptoReten['MontoImp'] != $datos['Encabezado']['Totales']['IVA']) {
                        $datos['Encabezado']['Totales']['IVANoRet'] =
                            $datos['Encabezado']['Totales']['IVA']
                            - $ImptoReten['MontoImp']
                        ;
                    }
                ...
                // si es adicional se suma al total
                    ...
                    ImpuestosAdicionales::getTipo($ImptoReten['TipoImp']) == 'A' &&
        ...
        if (!empty($datos['Encabezado']['Totales']['CredEC'])) {
            if ($datos['Encabezado']['Totales']['CredEC'] === true) {
                $datos['Encabezado']['Totales']['CredEC'] = round(
                    $datos['Encabezado']['Totales']['IVA'] * 0.65 // TODO: mover % a constante o método
                );
            }
        ...
        if (!in_array($datos['Encabezado']['IdDoc']['TipoDTE'], [39, 41])) {
            if (
                !empty($datos['Encabezado']['Totales']['MntTotal']) &&
                !empty($datos['Encabezado']['Totales']['MontoNF'])
            ) {
                $datos['Encabezado']['Totales']['MontoPeriodo'] =
                    $datos['Encabezado']['Totales']['MntTotal']
                    + $datos['Encabezado']['Totales']['MontoNF']
                ;
            }
        }
    }
    ...
    private function normalizar_boletas(array &$datos)
    {
        ...
        if (!empty($datos['Encabezado']['Receptor']['CorreoRecep'])) {
            $datos['Referencia'][] = [
                ...
                'RazonRef' => mb_substr(
                    'Email receptor: '.$datos['Encabezado']['Receptor']['CorreoRecep'], 0, 90
                ),
            ];
        }
    ...
    private function sanitizar(array &$datos)
    {
        ...
        $datos['Encabezado']['Emisor']['RUTEmisor'] = strtoupper(trim(str_replace(
            '.', '', $datos['Encabezado']['Emisor']['RUTEmisor']
        )));
        $datos['Encabezado']['Receptor']['RUTRecep'] = strtoupper(trim(str_replace(
            '.', '', $datos['Encabezado']['Receptor']['RUTRecep']
        )));
        $datos['Encabezado']['Receptor']['RznSocRecep'] = mb_substr(
            $datos['Encabezado']['Receptor']['RznSocRecep'], 0, 100
        );
        if (!empty($datos['Encabezado']['Receptor']['GiroRecep'])) {
            $datos['Encabezado']['Receptor']['GiroRecep'] = mb_substr(
                $datos['Encabezado']['Receptor']['GiroRecep'], 0, 40
            );
        }
        if (!empty($datos['Encabezado']['Receptor']['Contacto'])) {
            $datos['Encabezado']['Receptor']['Contacto'] = mb_substr(
                $datos['Encabezado']['Receptor']['Contacto'], 0, 80
            );
        }
        if (!empty($datos['Encabezado']['Receptor']['CorreoRecep'])) {
            $datos['Encabezado']['Receptor']['CorreoRecep'] = mb_substr(
                $datos['Encabezado']['Receptor']['CorreoRecep'], 0, 80
            );
        }
        if (!empty($datos['Encabezado']['Receptor']['DirRecep'])) {
            $datos['Encabezado']['Receptor']['DirRecep'] = mb_substr(
                $datos['Encabezado']['Receptor']['DirRecep'], 0, 70
            );
        }
        if (!empty($datos['Encabezado']['Receptor']['CmnaRecep'])) {
            $datos['Encabezado']['Receptor']['CmnaRecep'] = mb_substr(
                $datos['Encabezado']['Receptor']['CmnaRecep'], 0, 20
            );
        }
    ...
    {
        return (!$moneda || $moneda == 'PESO CL')
            ? (int)round($valor)
            : (float)round($valor, $decimal)
        ;
    }
    ...
    public function getEstadoValidacion(array $datos = null): int
    {
        ...
        if (is_array($datos)) {
            if (isset($datos['RUTEmisor']) && $this->getEmisor() != $datos['RUTEmisor']) {
                return 2;
            }
            if (isset($datos['RUTRecep']) && $this->getReceptor() != $datos['RUTRecep']) {
                return 3;
            }
        }
        return 0;
     ...
     * @return bool|null =true si la firma del DTE es válida, =null si no se pudo determinar.
     * @warning No se está verificando el valor del DigestValue del documento (solo la firma de ese DigestValue).
     */
     ...
     * @return bool =true si el documento es cedible.
     */
     ...
     * @return bool =true si el documento es una boleta electrónica.
     */
     ...
     * @return bool =true si el documento es una exportación.
     */
     ...
     * @return bool =true si el schema del documento del DTE es válido, =null si no se pudo validar.
     */
    public function schemaValidate()
    {
        // TODO: implementar método (solo puede validar si está firmado el DTE usando esquema SII).
        return true;
    }

    /**
     * Método que obtiene el estado del DTE.
     * @param Firma objeto que representa la Firma Electrónica.
     * @return array|false Arreglo con el estado del DTE.
     */
    public function getEstado(\libredte\lib\FirmaElectronica $Firma)
    {
        // solicitar token
        $token = \libredte\lib\Sii\Autenticacion::getToken($Firma);
        if (!$token) {
            return false;
        }
     ...
     * @return array|false Arreglo con el estado del DTE.
     */
    public function getEstadoAvanzado(\libredte\lib\FirmaElectronica $Firma)
    {
        // solicitar token
        $token = \libredte\lib\Sii\Autenticacion::getToken($Firma);
        if (!$token) {
        ...
        list($RutReceptor, $DvReceptor) = explode('-', $this->getReceptor());
        list($Y, $m, $d) = explode('-', $this->getFechaEmision());
        $xml = \libredte\lib\Sii::request('QueryEstDteAv', 'getEstDteAv', [
     ...
     * @return array|null Arreglo con los datos de la última acción.
     */
    public function getUltimaAccionRCV(\libredte\lib\FirmaElectronica $Firma)
    {
        list($emisor_rut, $emisor_dv) = explode('-', $this->getEmisor());
        $RCV = new \libredte\lib\Sii\RegistroCompraVenta($Firma);
        try {
            $eventos = $RCV->listarEventosHistDoc(
                $emisor_rut, $emisor_dv, $this->getTipo(), $this->getFolio()
            );
