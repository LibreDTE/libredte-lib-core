...
namespace libredte\lib\Sii\Dte\PDF;
 ...
 * @version 2020-08-21
 */
class Dte extends \libredte\lib\PDF
{

    use \libredte\lib\Sii\Dte\Base\DteImpreso;
     ...
     * @param posicion Posición respecto a datos del emisor (=0 izq, =1 arriba). Nota: parámetro válido solo para formato hoja carta
     * @version 2016-08-04
     */
     ...
     * Nota: método válido solo para formato hoja carta
     * @version 2016-08-05
     */
    ...
    protected function agregarEmisor(array $emisor, $x = 10, $y = 15, $w = 75, $w_img = 30, $font_size = null, array $color = null, $h_folio = null, $w_all = null)
    {
        ...
        if ($agregarDatosEmisor) {
            ...
            $ciudad = !empty($emisor['CiudadOrigen']) ? $emisor['CiudadOrigen'] : \libredte\lib\Chile::getCiudad($comuna);
    ...
    protected function agregarFolio($rut, $tipo, $folio, $sucursal_sii = null, $x = 130, $y = 15, $w = 70, $font_size = null, array $color = null)
    {
        ...
        if ($tipo) {
            $this->Texto('S.I.I. - '.\libredte\lib\Sii::getDireccionRegional($sucursal_sii), $x, $this->getY()+4, 'C', $w);
        }
        $this->SetTextColorArray([0,0,0]);
        $this->Ln();
        return $this->y;
    ...
    protected function agregarReceptor(array $Encabezado, $x = 10, $offset = 22)
    {
        ...
        if (!empty($receptor['DirRecep'])) {
            ...
            $ciudad = !empty($receptor['CiudadRecep']) ? $receptor['CiudadRecep'] : (
                !empty($receptor['CmnaRecep']) ? \libredte\lib\Chile::getCiudad($receptor['CmnaRecep']) : ''
            );
        ...
        if (!empty($receptor['Extranjero']['Nacionalidad'])) {
            ...
            $this->Texto(':', $x+$offset);
            $this->setFont('', '', null);
            $this->MultiTexto(\libredte\lib\Sii\Aduana::getNacionalidad($receptor['Extranjero']['Nacionalidad']), $x+$offset+2);
        }
    ...
    protected function agregarTraslado($IndTraslado, array $Transporte = null, $x = 10, $offset = 22)
    {
        ...
        if (!empty($Transporte['Aduana']) and is_array($Transporte['Aduana'])) {
            ...
            foreach ($Transporte['Aduana'] as $tag => $codigo) {
                ...
                $glosa = \libredte\lib\Sii\Aduana::getGlosa($tag);
                $valor = \libredte\lib\Sii\Aduana::getValor($tag, $codigo);
    ...
    protected function agregarTotales(array $totales, $otra_moneda, $y = 190, $x = 145, $offset = 25)
    {
        ...
        if (!empty($totales['ImptoReten'])) {
            ...
            foreach($ImptoReten as $i) {
                ...
                if (!empty($i['TasaImp'])) {
                    $glosas['ImptoReten_'.$i['TipoImp']] = \libredte\lib\Sii\ImpuestosAdicionales::getGlosa($i['TipoImp']).' ('.$i['TasaImp'].'%) $';
                } else {
                    $glosas['ImptoReten_'.$i['TipoImp']] = \libredte\lib\Sii\ImpuestosAdicionales::getGlosa($i['TipoImp']).' $';
                }
            }
