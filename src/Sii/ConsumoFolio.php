 ...
 */
class ConsumoFolio extends \libredte\lib\Sii\Base\Libro
{
    ...
    public function setCaratula(array $caratula)
    {
        ...
        $this->id = 'LibreDTE_CONSUMO_FOLIO_' . str_replace('-', '', $this->caratula['RutEmisor'])
            . '_' . str_replace('-', '', $this->caratula['FchInicio']) . '_' . date('U')
        ;
    }

    /**
     * Método que genera el XML del consumo de folios para el envío al SII.
     * @return string XML con el envio del consumo de folios firmado o =false si no se pudo generar o firmar el envío.
     */
    public function generar(): string
    {
        ...
        $this->xml_data = $this->Firma
            ? $this->Firma->signXML($xmlEnvio, '#'.$this->id, 'DocumentoConsumoFolios', true)
            : $xmlEnvio
        ;
        return $this->xml_data;
    }
     ...
     * @return string Fecha del primer documento que se está reportando.
     */
     ...
     * @return string Fecha del último documento que se está reportando.
     */
     ...
     * consumo de folios.
     * @return array Arreglo con los datos para generar los tags Resumen.
     */
     ...
     * @return array Arreglo con cada uno de los rangos de folios.
     */
    private function getRangos($folios): array
    {
