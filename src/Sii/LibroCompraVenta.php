 ...
 */
class LibroCompraVenta extends \libredte\lib\Sii\Base\Libro
{
    ...
    public function getID()
    {
        ...
        return !empty($this->datos['LibroCompraVenta']['EnvioLibro']['@attributes']['ID'])
            ? $this->datos['LibroCompraVenta']['EnvioLibro']['@attributes']['ID']
            : false
        ;
    }
     ...
     * @return bool =true al agregar detalle o =false si no se agrega (no hay tipo de documento en el detalle que se desea agregar).
     */
    public function agregar(array $detalle, $normalizar = true): bool
    {
        if ($normalizar) {
    ...
    private function normalizarDetalle(array &$detalle)
    {
        ...
        // si el caso está anulado se genera solo lo mínimo pedido por el SII
        ...
        } else if (!$detalle['MntIVA'] && !is_array($detalle['IVANoRec']) && $detalle['TasaImp'] && $detalle['MntNeto']) {
        ...
        if ($detalle['MntExe'] === false && $detalle['MntNeto'] === false && $detalle['MntTotal'] === 0) {
            $detalle['MntNeto'] = 0;
        }
        // colocar montos que falten en 0 si es solo exento y no neto
        if ($detalle['MntNeto'] === false && $detalle['MntExe']) {
            $detalle['TasaImp'] = \libredte\lib\Sii::getIVA();
            $detalle['MntNeto'] = 0;
            $detalle['MntIVA'] = 0;
        }
     ...
     *   6: FchDoc
     *   7: Anulado (opcional, 'A' solo para folios anulados, no anulados con NC o ND)
     *   8: MntExe (opcional)
     *   9: MntNeto (opcional)
     ...
     *   26: NumInt (opcional)
     *   27: Emisor (opcional, '1' solo si es NC o ND de FC emitida por el emisor del libro)
     ...
     */
    public function agregarComprasCSV($archivo, $separador = ';', $delimitadortexto = '"')
    {
        $data = \libredte\lib\CSV::read($archivo, $separador, $delimitadortexto);
        $n_data = count($data);
        for ($i=1; $i<$n_data; $i++) {
            // detalle genérico
            $detalle = [
            ...
            if (!empty($data[$i][11])) {
                $detalle['IVANoRec'] = [
                    ...
                    'MntIVANoRec' => !empty($data[$i][12])
                        ? $data[$i][12]
                        : round((int)$detalle['MntNeto'] * ((float)$detalle['TasaImp']/100))
                    ,
                ];
            }
            // agregar código y monto de otros impuestos
            if (!empty($data[$i][14]) && (!empty($data[$i][15]) || !empty($data[$i][16]))) {
                $detalle['OtrosImp'] = [
                    'CodImp' => $data[$i][14],
                    'TasaImp' => !empty($data[$i][15]) ? $data[$i][15] : 0,
                    'MntImp' => !empty($data[$i][16])
                        ? $data[$i][16]
                        : round($detalle['MntNeto'] * ($data[$i][15]/100))
                    ,
                ];
            }
     ...
     *   4: RznSoc (opcional)
     *   5: FchDoc
     *   6: Anulado (opcional, 'A' solo para folios anulados, no anulados con NC o ND)
     *   7: MntExe (opcional)
     *   8: MntNeto (opcional)
     *   9: MntIVA (calculable)
     *   10: IVAFueraPlazo
     ...
     *   26: PsjNac
     *   27: PsjInt
     *   Extranjero (solo DTE de exportación):
     *     28: NumId
     *     29: Nacionalidad
     ...
     * @param separador Separador de campos del archivo CSV.
     */
    public function agregarVentasCSV($archivo, $separador = ';', $delimitadortexto = '"')
    {
        $data = \libredte\lib\CSV::read($archivo, $separador, $delimitadortexto);
        $n_data = count($data);
        for ($i=1; $i<$n_data; $i++) {
            ...
            if (!empty($data[$i][11])) {
                $detalle['OtrosImp'] = [
                    ...
                    'MntImp' => !empty($data[$i][13])
                        ? $data[$i][13]
                        : round($detalle['MntNeto'] * ($data[$i][12]/100))
                    ,
                ];
            }
            // agregar datos extranjeros
            if (!empty($data[$i][28]) || !empty($data[$i][29])) {
                $detalle['Extranjero'] = [
    ...
    public function setCaratula(array $caratula)
    {
        ...
        $this->id = 'LibreDTE_LIBRO_' . $this->caratula['TipoOperacion'] . '_'
            . str_replace('-', '', $this->caratula['RutEmisorLibro']) . '_'
            . str_replace('-', '', $this->caratula['PeriodoTributario']) . '_' . date('U')
        ;
    }

    /**
     * Método que genera el XML del libro IECV para el envío al SII.
     * @param incluirDetalle =false no se incluirá el detalle de los DTEs (solo se usará para calcular totales).
     * @return string XML con el envio del libro de compra y venta firmado o =false si no se pudo generar o firmar el envío.
     */
    public function generar($incluirDetalle = true)
    {
        ...
        $xmlEnvio = (new \libredte\lib\XML())->generate([
            'LibroCompraVenta' => [
                '@attributes' => [
                    ...
                    'xsi:schemaLocation' => $this->simplificado
        ...
        $this->xml_data = (!$this->simplificado && $this->Firma)
            ? $this->Firma->signXML($xmlEnvio, '#'.$this->id, 'EnvioLibro', true)
            : $xmlEnvio
        ;
        return $this->xml_data;
    }
    ...
    /**
     * Método que permite agregar solo resumen al libro (sin detalle), esto para
     ...
     * @return bool =true si se asignó el resumen, =false si no se asignó por faltar tipo de documento o total que es lo mínimo requerido.
     */
    public function setResumen(array $resumen): bool
    {
        // verificar que se haya pasado el tipo de documento y total como mínimo
        foreach ($resumen as $tipo) {
            if (!isset($tipo['TpoDoc']) || !isset($tipo['TotDoc'])) {
                return false;
            }
        }
        // asignar resumen
     ...
     * @return array Arreglo con los datos para generar los tags TotalesPeriodo.
     */
    ...
    public function getResumenManual(): array
    {
        $manual = [];
        $libro = $this->toArray();
        if (isset($libro['LibroCompraVenta']['EnvioLibro']['ResumenPeriodo']['TotalesPeriodo'])) {
            $totales = $libro['LibroCompraVenta']['EnvioLibro']['ResumenPeriodo']['TotalesPeriodo'];
            if (!isset($totales[0])) {
                $totales = [$totales];
            }
            foreach ($totales as $total) {
                if (isset($total['TpoDoc']) && in_array($total['TpoDoc'], [35, 38, 48])) {
     ...
     */
    public function getResumenBoletas(): array
    {
        $manual = [];
        $libro = $this->toArray();
        if (isset($libro['LibroCompraVenta']['EnvioLibro']['ResumenPeriodo']['TotalesPeriodo'])) {
            ...
            foreach ($totales as $total) {
                if (in_array($total['TpoDoc'], [39, 41])) {
                    $manual[$total['TpoDoc']] = array_merge(
                        $this->total_default,
                        $total
                    );
                }
     ...
     * el archivo CSV.
     * @return array Arreglo con los datos de las compras.
     */
    public function getCompras(): array
    {
        ...
        foreach ($this->detalles as $d) {
            ...
            $detalle[] = [
                ...
                (isset($d['FctProp']) && $d['FctProp'] !== false) ? $d['FctProp'] : null,
            ];
        }
        return $detalle;
    }
