...
namespace libredte\lib\Sii\Certificacion;
...
class SetPruebas
{
    ...
    private static $referencias = [
        ...
        'CORRIGE' => [
            'codigo' => 2,
            'detalle' => 2, // =2 copia solo nombre del primer item
            'Totales' => [
    ...
    public static function getJSON($archivo, array $folios = [], $separador = '==============')
    {
        ...
        foreach ($casos as $caso) {
                ...
                if (isset($caso['exportacion'])) {
                    ...
                    if (!empty($caso['exportacion']['NACIONALIDAD'])) {
                        $documento['Encabezado']['Receptor']['Extranjero'] = [
                            'Nacionalidad' => \libredte\lib\Sii\Aduana::getCodigo(
                    ...
                    foreach ($caso['exportacion'] as $var => $val) {
                        if (isset(self::$Aduana[$var])) {
                            $tag = self::$Aduana[$var];
                            $valor = \libredte\lib\Sii\Aduana::getCodigo($tag, $val);
            ...
            // otro caso que se esté referenciando
                ...
                // copiar solo el nombre del primer item
            ...
            // agregar recargo por flete y/o seguro, se agrega solo en factura
            ...
            if (!empty($caso['referencia'])) {
                ...
                if (isset($documento['Encabezado']['Totales']['TasaIVA']))
                    $documento['Encabezado']['Totales']['TasaIVA'] = \libredte\lib\Sii::getIVA();
            ...
            // si hay Totales pero no hay valores en los detalles entonces se cambia a solo Totales de MntTotal = 0
    ...
    private static function parse($archivo, $separador)
    {
        ...
        foreach ($casos as $caso) {
            ...
            // solo continuar si hay más líneas, ya que serán detalles
            if (isset($lineas[$linea_titulos_detalles])) {
                ...
                // solo continuar si hay más líneas, será, por ej, el descuento global
                if (isset($lineas[$i])) {
