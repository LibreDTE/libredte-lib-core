 ...
 */
class LibroGuia extends \libredte\lib\Sii\Base\Libro
{

    /**
     * Método que agrega un detalle al listado que se enviará
     * @param detalle Arreglo con el resumen del DTE que se desea agregar
     * @return bool =true al agregar detalle (no hay máximo, por eso no entrega nunca =false)
     */
    public function agregar(array $detalle, bool $normalizar = true): bool
    {
        if ($normalizar) {
     ...
     * @param detalle Arreglo con el resumen del DTE que se desea agregar.
     * @return array Arreglo con el detalle normalizado.
     */
     ...
     */
    public function agregarCSV($archivo, $separador = ';', $delimitadortexto = '"')
    {
    ...
    public function setCaratula(array $caratula)
    {
        ...
        $this->id = 'LibreDTE_LIBRO_GUIA_' . str_replace('-', '', $this->caratula['RutEmisorLibro'])
            . '_' . str_replace('-', '', $this->caratula['PeriodoTributario']) . '_' . date('U');
    }

    /**
     * Método que genera el XML del libro IECV para el envío al SII.
     * @param incluirDetalle =false los detalles solo se usarán para calcular totales (sin incluirlos).
     * @return string XML con el envio del libro de guías de despacho firmado o =false si no se pudo generar o firmar el envío.
     */
     ...
     */
    private function getResumenPeriodo(): array
    {
        $ResumenPeriodo = [
            'TotFolAnulado' => false,
