 ...
 */
class RespuestaEnvio extends \libredte\lib\Sii\Base\Documento
{
     ...
     * @return bool =true si se pudo agregar la respuesta o =false si no se agregó por exceder el límite o bien ya existía al menos una respuesta de documento.
     */
    public function agregarRespuestaEnvio(array $datos): bool
    {
     ...
     * @param datos Arreglo con los datos de la respuesta.
     * @return bool =true si se pudo agregar la respuesta o =false si no se agregó por exceder el límite o bien ya existía al menos una respuesta de envío.
     */
    public function agregarRespuestaDocumento(array $datos): bool
    {
     ...
     * @return string XML con la respuesta firmada o =false si no se pudo generar o firmar la respuesta.
     */
    public function generar()
    {
        ...
        if (!isset($this->respuesta_envios[0]) && !isset($this->respuesta_documentos[0])) {
            \libredte\lib\Log::write(
                \libredte\lib\Estado::RESPUESTAENVIO_FALTA_RESPUESTA,
                \libredte\lib\Estado::get(\libredte\lib\Estado::RESPUESTAENVIO_FALTA_RESPUESTA)
            );
            return false;
        }
        ...
        if (!$this->caratula) {
            \libredte\lib\Log::write(
                \libredte\lib\Estado::RESPUESTAENVIO_FALTA_CARATULA,
                \libredte\lib\Estado::get(\libredte\lib\Estado::RESPUESTAENVIO_FALTA_CARATULA)
            );
            return false;
        }
        ...
        $xmlEnvio = (new \libredte\lib\XML())->generate($arreglo)->saveXML();
        // firmar XML del envío y entregar
        $this->xml_data = $this->Firma
            ? $this->Firma->signXML($xmlEnvio, '#LibreDTE_ResultadoEnvio', 'Resultado', true)
            : $xmlEnvio
        ;
        return $this->xml_data;
    }
     ...
     */
    public function esRecepcionEnvio(): bool
    {
        return isset($this->arreglo['RespuestaDTE']['Resultado']['RecepcionEnvio']);
    }

    /**
     * Método que indica si el XML corresponde a ResultadoDTE.
     */
    public function esResultadoDTE(): bool
    {
    ...
    public function getRecepciones()
    {
        ...
        if (!$this->respuesta_envios) {
            ...
            // procesar solo si hay recepciones
