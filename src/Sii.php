...
/**
 * Clase para acciones genéricas asociadas a procesos de facturación con
 * software de mercado del SII en Chile.
 */
class Sii
{
     ...
     * @return string URL del WSDL del servicio según ambiente solicitado.
     */
    public static function wsdl($servicio, $ambiente = null)
    {
        ...
        $location = isset(self::$config['wsdl'][$servicio])
            ? self::$config['wsdl'][$servicio] : self::$config['wsdl']['*'];
        $wsdl = str_replace(
            ['{servidor}', '{servicio}'],
    ...
    public static function request($wsdl, $request, $args = null, $retry = null)
    {
        ...
        try {
            ...
            Log::write(
                Estado::REQUEST_ERROR_SOAP,
                Estado::get(Estado::REQUEST_ERROR_SOAP, $msg)
            );
            return false;
        }
        for ($i=0; $i<$retry; $i++) {
        ...
        if ($body === null) {
            Log::write(
                Estado::REQUEST_ERROR_BODY,
                Estado::get(Estado::REQUEST_ERROR_BODY, $wsdl, $retry)
            );
            return false;
        }
    ...
    public static function enviar($usuario, $empresa, $dte, $token, $gzip = false, $retry = null)
    {
        ...
        try {
            ...
            Log::write(
                ...
                Estado::get(Estado::ENVIO_ERROR_XML, $e->getMessage())
            );
            return false;
        }
        if ($xml->STATUS != 0) {
            Log::write(
                $xml->STATUS,
                Estado::get($xml->STATUS)
                    . (isset($xml->DETAIL) ? ('. ' . implode("\n", (array)$xml->DETAIL->ERROR)) : '')
            );
        }
        return $xml;
    }
    ...
    public static function cert($idk = null)
    {
        ...
        if (!is_readable($cert)) {
            Log::write(
                Estado::SII_ERROR_CERTIFICADO,
                Estado::get(Estado::SII_ERROR_CERTIFICADO, self::$config['certs'][$ambiente])
            );
            return false;
        }
        return file_get_contents($cert);
    ...
    /**
     * Método que asigna el ambiente que se usará por defecto.
     * No se verifica el certificado SSL en ambiente de certificación.
     * @param ambiente Ambiente a usar: Sii::PRODUCCION o Sii::CERTIFICACION
     */
    public static function setAmbiente($ambiente = self::PRODUCCION): int
    {
        $ambiente = $ambiente ? self::CERTIFICACION : self::PRODUCCION;
        if ($ambiente == self::CERTIFICACION) {
            self::setVerificarSSL(false);
        }
        self::$ambiente = (int)(bool)$ambiente;
     ...
     */
    public static function getAmbiente($ambiente = null): int
    {
        if ($ambiente === null) {
            $ambiente = self::$ambiente;
        }
        return (int)(bool)$ambiente;
    }

    /**
     * Método que entrega la tasa de IVA vigente.
     * @return float Tasa de IVA vigente.
     */
     ...
     *
     * Si el argumento es una cadena no numérica, intenta encontrar una dirección regional correspondiente
     * en el arreglo de direcciones. Si no se encuentra, devuelve la cadena original en mayúsculas.
     * Si el argumento es numérico, asume que es un código de sucursal y devuelve un formato de sucursal.
     * Si el argumento es falso o vacío, devuelve 'N.N.' como valor por defecto.
     *
     * @param mixed $comuna El nombre de la comuna o el código de la sucursal.
     * @return string La dirección regional correspondiente, un formato de sucursal para códigos numéricos,
     *                la misma entrada en mayúsculas si no se encuentra en el arreglo, o 'N.N.' si la entrada
     *                es falsa o vacía.
     */
    public static function getDireccionRegional($comuna): string
    {
        if (!$comuna) {
            return 'N.N.';
        }
        if (!is_numeric($comuna)) {
            ...
            return self::$direcciones_regionales[$comuna] ?? $comuna;
        }
        return 'SUC ' . $comuna;
    }
